...
if (args.direction !== 'up' && args.direction !== 'right' && args.direction !== 'down' && args.direction !== 'left') {
	args.direction = 'up';
}
...
function doTouch (e) {
	if (/^__child\d+$/.test (e.source.id)) {
		...
		e.source.getChildren() [0].applyProperties ({
			//borderColor: e.type === 'touchstart' ? args.items[index].focusedColor :
			// args.items[index].backgroundColor || '#ccfafafa'
		});
	}
	else {
		...
		e.source.getChildren() [0].applyProperties ({
			//borderColor: e.type === 'touchstart' ? args.focusedColor : args.backgroundColor
			// || '#ccfafafa'
		});
	}
}
...
var widget = {
	...
	backgroundImage : _.has (args, 'backgroundImage') ? args.backgroundImage : ""
},
    circle = {
	width : _.has (args, 'width') ? args.width : WIDTH,
	height : _.has (args, 'height') ? args.height : HEIGHT,
	...
	borderColor : _.has (args, 'backgroundColor') ? args.backgroundColor : 'transparent',
	backgroundImage : _.has (args, 'backgroundImage') ? args.backgroundImage : ""
},
    title = {
	width : _.has (args, 'width') ? args.width : WIDTH,
...
_.each (args.items, function (item, index) {
	...
	container.add (Ti.UI.createView (_.extend (circle, {
		...
		borderColor : _.has (item, 'backgroundColor') ? item.backgroundColor : 'transparent',
		opacity : 1.0,
		touchEnabled : false
	})));
